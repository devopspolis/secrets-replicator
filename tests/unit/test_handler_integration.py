...
def create_mock_tags_and_transform_client(
    tags=None, transform_secret_name="my-transform-sed", sed_script="s/us-east-1/us-west-2/g"
):
    ...
    # Set up tags
    default_tags = (
        {"SecretsReplicator:TransformSecretName": transform_secret_name}
...
class TestHandlerAWSIntegration:
    ...
    @patch.dict(os.environ, {"DEST_REGION": "us-west-2", "TRANSFORM_MODE": "sed"})
    ...
    @patch.dict(os.environ, {"DEST_REGION": "us-west-2", "TRANSFORM_MODE": "json"})
    @patch("handler.create_secrets_manager_client")
...
class TestPassThroughReplication:
    ...
    @patch.dict(os.environ, {"DEST_REGION": "us-west-2"})
    @patch("handler.create_secrets_manager_client")
    def test_passthrough_replication_no_tag(self, mock_create_client):
        ...
        assert (
            call_kwargs["secret_value"] == '{"host":"db.example.com","port":"5432"}'
        )  # Original value
    ...
    @patch.dict(os.environ, {"DEST_REGION": "us-east-1"})  # Same region
    @patch("handler.create_secrets_manager_client")
