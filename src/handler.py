...
from event_parser import (
    parse_eventbridge_event, validate_event_for_replication, EventParsingError,
    is_manual_trigger, parse_manual_event, validate_manual_event_for_replication,
...
def handle_manual_sync(
    ...
    try:
        ...
        log_event(logger, 'INFO', f'Parsed {len(secret_events)} secret(s) from manual event',
        ...
        return {
            'statusCode': 400,
            'body': f'Invalid manual sync event: {e}'
        }
    ...
    for i, secret_event in enumerate(secret_events, 1):
        log_event(logger, 'INFO', f'Processing secret {i}/{len(secret_events)}',
                 secret_id=secret_event.secret_id)

        # Validate the manual event
        if not validate_manual_event_for_replication(secret_event):
            log_event(logger, 'WARNING', 'Secret event validation failed',
                     secret_id=secret_event.secret_id)
            all_results.append({
                'secretId': secret_event.secret_id,
                'statusCode': 400,
    ...
    log_event(logger, 'INFO', 'Manual sync completed',
